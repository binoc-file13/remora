<?php
    ...
    class productDetails {
        ...
        function productDetails() {
            ...
            $this->download_base_url_direct = 'http://download.mozilla.org/';
        ...
        function getFilesizeForLocaleAndPlatform($locale, $platform) {
            ...
            if (array_key_exists('filesize', $this->primary_builds[$locale][$_current_version][$platform])) {
        ...
        function getNewestVersionForLocaleFromBuildArray($locale, $build_array) {
            ...
            if (isset($build_array[$locale]) && is_array($build_array[$locale]) && !empty($build_array[$locale])) {
                foreach (array_reverse($build_array[$locale]) as $version => $platforms) {
                    ...
                    if ($version == LATEST_FIREFOX_DEVEL_VERSION) {
                        continue;
                    }
        ...
        function getDownloadBlockForLocale($locale, $options=array()) {
            ...
            if (array_key_exists('Linux', $this->primary_builds[$locale][$_current_version]) || array_key_exists('Linux', $this->beta_builds[$locale][$_current_version])) {
                $_li_linux = $this->_getDownloadBlockListHtmlForLocaleAndPlatform($locale, 'Linux', $options);
            }
            if (array_key_exists('OS X', $this->primary_builds[$locale][$_current_version]) || array_key_exists('OS X', $this->beta_builds[$locale][$_current_version])) {
                ...
                $_li_osx = $this->_getDownloadBlockListHtmlForLocaleAndPlatform($_t_locale, 'OS X', $options);
            }
            ...
            switch ($_layout) {
                case 'sidebar':
                    ...
                    <li class="{$_os_class}">
                        ...
                        <div>{$_current_version}, {$_language_name}, {$_os_name} ({$_os_filesize}<abbr title="{$_megabytes}">{$_mb}</abbr>)</div>
...
LI_MAIN;
        ...
        function _getDownloadTableFromBuildArray($build_array, $options=array()) {
            ...
            foreach ($build_array as $locale => $versions) {
                ...
                if ( ($_build_version == $_latest_version) && ($_product_version == 'oldest') ) {
                    continue;
                }
                ...
                if (array_key_exists('Windows', $build_array[$locale][$_build_version])) {
                    $_onclick = ($_include_init_download) ?  "onclick=\"init_download('{$this->download_base_url_direct}?product={$_product}-{$_build_version}&amp;os=win&amp;lang={$locale}');\"" : '';
                    $_windows_link = "<a {$_onclick} href=\"{$_download_base_url}?product={$_product}-{$_build_version}&amp;os=win&amp;lang={$locale}\" class=\"download-windows\">{$_download}</a>";
                }
                if (array_key_exists('Linux', $build_array[$locale][$_build_version])) {
                    $_onclick = ($_include_init_download) ?  "onclick=\"init_download('{$this->download_base_url_direct}?product={$_product}-{$_build_version}&amp;os=linux&amp;lang={$locale}');\"" : '';
                    $_linux_link = "<a {$_onclick} href=\"{$_download_base_url}?product={$_product}-{$_build_version}&amp;os=linux&amp;lang={$locale}\" class=\"download-linux\">{$_download}</a>";
                }
                if (array_key_exists('OS X', $build_array[$locale][$_build_version])) {
                    // Special case for OS X and Japanese...
                    $_t_locale = ($locale == 'ja') ? 'ja-JP-mac' : $locale;
                    $_onclick = ($_include_init_download) ?  "onclick=\"init_download('{$this->download_base_url_direct}?product={$_product}-{$_build_version}&amp;os=osx&amp;lang={$locale}');\"" : '';
                    $_osx_link = "<a {$_onclick} href=\"{$_download_base_url}?product={$_product}-{$_build_version}&amp;os=osx&amp;lang={$_t_locale}\" class=\"download-osx-uni\">{$_download}</a>";
                }
...
JS_INCLUDE;
    ...
    <table class="downloads dalvay-table" cellpadding="0" cellspacing="0" summary="{$_table_summary}">
        <thead>
            <tr>
            <th colspan="2" class="top-left">{$_language}</th>
            <th>{$_version}</th>
            <th><span class="download-windows">{$_windows_name}</span></th>
            <th><span class="download-osx-uni">{$_osx_name}</span></th>
...
TABLE_LAYOUT;
 ...
 \t"{$region}": { fx: {$_fx},\tfxold: {$_fxold},\tfxbeta: {$_fxbeta},\ttb: {$_tb},\ttbold: {$_tbold},\ttbbeta: {$_tbbeta},\tname: "{$data['name']}",\tlocalName: "{$data['localName']}" }
A_FINAL;
            }
         ...
         *
         */
        function getNoScriptBlockForLocale($locale, $options) {
            $_product         = array_key_exists('product', $options) ?  $options['product'] : 'firefox';
            $_devel_version   = array_key_exists('devel_version', $options) ? $options['devel_version'] : false;
            ...
            $_download_product            = ___('Download Now - Free');
            $_other_systems_and_languages = ___('Other Systems and Languages');
            $_windows_name                = ___('Windows');
            $_linux_name                  = ___('Linux');
            $_osx_name                    = ___('Mac OS X');
            ...
            if (array_key_exists('Windows', $this->primary_builds[$locale][$_current_version]) || array_key_exists('Windows', $this->beta_builds[$locale][$_current_version])) {
                ...
                <li><a href="http://download.mozilla.org/?product={$_product}-{$_current_version}&amp;os=win&amp;lang={$locale}">{$_windows_name} ({$_windows_filesize}<abbr title="{$_megabytes}">{$_mb}</abbr>)</a></li>
LI_WINDOWS;
            }
            if (array_key_exists('Linux', $this->primary_builds[$locale][$_current_version]) || array_key_exists('Linux', $this->beta_builds[$locale][$_current_version])) {
                $_li_linux = <<<LI_LINUX
                <li><a href="http://download.mozilla.org/?product={$_product}-{$_current_version}&amp;os=linux&amp;lang={$locale}">{$_linux_name} ({$_linux_filesize}<abbr title="{$_megabytes}">{$_mb}</abbr>)</a></li>
LI_LINUX;
            }
            if (array_key_exists('OS X', $this->primary_builds[$locale][$_current_version]) || array_key_exists('OS X', $this->beta_builds[$locale][$_current_version])) {
                ...
                $_li_osx = <<<LI_OSX
                <li><a href="http://download.mozilla.org/?product={$_product}-{$_current_version}&amp;os=osx&amp;lang={$locale}">{$_osx_name} ({$_osx_filesize}<abbr title="{$_megabytes}">{$_mb}</abbr>)</a></li>
LI_OSX;
            }
